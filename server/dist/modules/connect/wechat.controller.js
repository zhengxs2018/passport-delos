"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.WeChatController = void 0;
const openapi = require("@nestjs/swagger");
const common_1 = require("@nestjs/common");
const auth_service_1 = require("../auth/auth.service");
const wechat_guard_1 = require("./guards/wechat.guard");
let WeChatController = class WeChatController {
    constructor(authService) {
        this.authService = authService;
    }
    authorize(req) {
        return this.authService.genToken(req.user);
    }
    callback(req) {
        return this.authService.genToken(req.user);
    }
};
__decorate([
    common_1.Get('wechat'),
    common_1.UseGuards(wechat_guard_1.WeChatAuthGuard),
    openapi.ApiResponse({ status: 200 }),
    __param(0, common_1.Req()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", void 0)
], WeChatController.prototype, "authorize", null);
__decorate([
    common_1.Get('wechat/callback'),
    common_1.UseGuards(wechat_guard_1.WeChatAuthGuard),
    openapi.ApiResponse({ status: 200 }),
    __param(0, common_1.Req()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", void 0)
], WeChatController.prototype, "callback", null);
WeChatController = __decorate([
    common_1.Controller('connect'),
    __metadata("design:paramtypes", [auth_service_1.AuthService])
], WeChatController);
exports.WeChatController = WeChatController;
//# sourceMappingURL=wechat.controller.js.map